...
/**
 * Attache le WAV (chemin + analyse « vérité terrain ») à sa ligne d'appel, par
 * Twilio CallSid. On essaie d'abord `calls` (appels planifiés + entrants AICOUTE),
 * puis `demo_calls` (démos vitrine/test) — les SID Twilio sont uniques, pas de
 ...
 * @param {object|null} analysis  sortie de analyzeDualChannelWav (ou null)
 */
export async function attachRecordingPath(callSid, path, analysis = null) {
  if (!supabase || !callSid || !path) return
  const patch = { recording_path: path }
  if (analysis) patch.recording_analysis = analysis
  try {
    const { data, error } = await supabase
      .from('calls')
      .update(patch)
    ...
    await supabase.from('demo_calls').update(patch).eq('twilio_call_sid', callSid)
  } catch (err) {
