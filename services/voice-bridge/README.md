...
- `MAX_CALL_SECONDS` (filet de sécurité, défaut 120 = 2 min)
...
- **Coupure serveur** à `MAX_CALL_SECONDS` (120 s = 2 min par défaut) — même si Twilio ou OpenAI bug, l'appel se termine et la facture est plafonnée.
- **Numéros loggés masqués** (`+33 6•• ••• •78`) pour éviter de stocker des numéros complets en clair dans les logs.

## Coûts

Par démo (~2 min, limite stricte) :
- Twilio FR mobile sortant : ~0,06 €
- OpenAI Realtime : ~0,40 €
- **Total : ~0,46 € par démo téléphonique**
