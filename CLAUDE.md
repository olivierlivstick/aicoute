...
| `/admin/qualite` | **Stats de fluidité agrégées** (temps réel) sur `calls.fluidity_metrics`. Filtres : périmètre (global / par aidant / par bénéficiaire) + période (8 / 30 j). KPIs combinés + **comparaison OpenAI vs Gemini** (le « blanc » en conversation est poolé depuis les `samples_ms` bruts de tous les appels du périmètre → vraie distribution ; repère `~` = approx Gemini). Lecture directe `calls` (RLS admin). | [AdminQualite.tsx](apps/web/src/pages/admin/AdminQualite.tsx) |
