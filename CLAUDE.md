...
| `/admin/sante` | Calls bloqués (`notified` > 5 min · `in_progress` > 30 min · `scheduled` retry > 5 min) + dernier appel terminé + auto-refresh 30s + **section « Veille modèles voix »** (CTA → Edge Fn `model-watch`, recherche web, cf. note Veille plus haut) + **section « Diagnostic fluidité »** (cf. ci-dessous) + **Événements système** (50 derniers `system_events`) | [AdminSante.tsx](apps/web/src/pages/admin/AdminSante.tsx) |
