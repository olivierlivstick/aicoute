...
- `minute_purchases` (2026-06-09) — achats de **packs de minutes** (nouveau modèle de crédit) : `caregiver_id`, `pack_id`, `pack_name`, `minutes`, `amount_eur`, `created_at`. Alimente l'onglet « Mes achats » de `/compte` + le calcul des **minutes disponibles** (`stock offert essai + acheté − consommé`, hook `useMinutesBalance`). **Vide tant que le paiement n'est pas branché (Stripe = phase 2)** ; la lecture front dégrade proprement si la table manque. RLS : `caregiver_owns_purchases` (ALL) + `admin_select_purchases` ; **GRANT** SELECT/INSERT `authenticated` (migration brute). ⚠️ Le modèle « crédit minutes » n'est PAS encore câblé en temps réel (pas de décompte, pas de bridage par solde) — `subscriptions`/`PLAN_TIERS` (appels/semaine) coexistent encore. Catalogue des packs : `MINUTE_PACKS` dans `packages/shared/src/types.ts` (source UNIQUE partagée vitrine `#tarifs` + back-office `PlanChooser`).
...
| `/compte` | **2 onglets** : Mon profil / **Mes achats** (liste `minute_purchases`, vide tant que le paiement n'est pas branché) + **carte « Minutes disponibles »** en haut (stock offert essai + acheté − consommé ; hook [useMinutesBalance.ts](apps/web/src/hooks/useMinutesBalance.ts)). « Mon abonnement » retiré. | [ComptePage.tsx](apps/web/src/pages/compte/ComptePage.tsx) |
