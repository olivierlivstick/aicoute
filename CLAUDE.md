...
| `/contexte` | Profil bénéficiaire en **6 onglets** (Infos / Histoire / Goûts / Personnalité / Configuration IA / **Mémoire**), save inline par section. Onglet Configuration IA = persona/**voix Gemini (écoute d'échantillons via [VoicePicker.tsx](apps/web/src/components/VoicePicker.tsx), 6 vignettes compactes sur une ligne)**/style (4 vignettes alignées)/langue — **plus de choix de moteur (Gemini-only)** + **Prompt de personnalité** (`custom_prompt`, éditable, bouton « Réinitialiser depuis le défaut »). Onglet Mémoire = souvenirs `conversation_memory` consultables/éditables. | [ContextePage.tsx](apps/web/src/pages/contexte/ContextePage.tsx) + [BeneficiaryContextEditor.tsx](apps/web/src/pages/contexte/BeneficiaryContextEditor.tsx) |
