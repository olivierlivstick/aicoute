...
### Layout commun
- **Sidebar gauche** ([AppLayout.tsx](apps/web/src/components/AppLayout.tsx)) : Tableau de bord / **Bénéficiaires** (route `/contexte`) / **Appels** (route `/historique`) / Veille + section « Mon compte » séparée en bas. (Renommages 2026-06-09 ; **« Planning » retiré du menu** → devenu un onglet de la fiche bénéficiaire.)
...
| `/admin/sante` | **3 onglets**. **Santé** : calls bloqués (`notified` > 5 min · `in_progress` > 30 min · `scheduled` retry > 5 min) + dernier appel terminé + auto-refresh 30s + **section « Diagnostic fluidité »** (cf. ci-dessous) + **Événements système** (50 derniers `system_events`). **Veille** : section « Veille modèles voix » (CTA → Edge Fn `model-watch`, recherche web) + « Test d'appel OpenAI / Gemini » (appel téléphone de test, moteur+langue+ouverture, via `/call`, tracé `demo_calls` ; seul endroit où OpenAI reste essayable). **Fine-tuning** : édition à chaud des seuils d'analyse WAV + VAD live Gemini/OpenAI (`app_settings.fluidity_tuning`, lu par le voice-bridge via cache `tuning.js` — cf. section « Réglages env » de l'archi vocale). | [AdminSante.tsx](apps/web/src/pages/admin/AdminSante.tsx) |
...
`/planning → /contexte?tab=planning` (page Planning supprimée → onglet), `/sessions → /contexte?tab=planning`, `/reports → /historique`, `/reports/:id → /historique/:id`, `/settings → /compte`, `/beneficiary → /contexte`, `/beneficiary/:id → /contexte` (avec sélection auto), `/memories → /dashboard`, `/setup → /compte`.
...
Paiement réel des **packs de minutes** (`MINUTE_PACKS`) via **Stripe Checkout (paiement unique)**, branché derrière la section TARIFS de la home ET le back-office. Distinct du modèle `subscriptions`/`PLAN_TIERS` (appels/semaine) qui coexiste encore. **État (2026-06-10)** : tout le code + la base + les Edge Functions sont **déployés en prod** ; il ne manque que le secret `STRIPE_SECRET_KEY` (clé restreinte `rk_live_` en attente de déblocage 2FA Stripe) → tant qu'il est absent, `create-checkout-session` renvoie 500 « STRIPE_SECRET_KEY non défini ».
...
ALLOWED_ORIGINS=https://www.aicoute.fr,https://aicoute.fr,https://app.aicoute.fr,https://app.modect.com  # CRITIQUE : doit inclure (1) les origines qui ouvriront /ws/gemini-web ET (2) l'origine du BACK-OFFICE pour le test d'appel admin /admin/sante (POST /call). ⚠️ Le back-office est servi en prod sur app.modect.com (ancien domaine, pas encore basculé sur app.aicoute.fr) → c'est app.modect.com l'origine réelle. Sans elle → preflight sans Access-Control-Allow-Origin → « Failed to fetch » côté navigateur
