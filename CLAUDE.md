...
| `/admin/appels` | Tous les appels, filtres URL (`?period=`, `?status=`, `?severity=high`). Onglet passés : colonnes Planifié (`scheduled_at`) / Effectif (`started_at`, fallback `notified_at`) + Coût IA + Coût Twilio (réel si remonté, sinon estimation `~` par la durée). Actions : « Relancer » sur missed/failed, « Déclencher maintenant » sur prévus, « Supprimer » (avec confirmation) sur les appels prévus | [AdminAppels.tsx](apps/web/src/pages/admin/AdminAppels.tsx) |
