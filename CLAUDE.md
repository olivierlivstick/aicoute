...
Outil de **vérité terrain** pour valider/calibrer les métriques de fluidité, piloté **sans CLI/env** depuis la section « Diagnostic fluidité » de `/admin/sante` ([AdminSante.tsx](apps/web/src/pages/admin/AdminSante.tsx), composant `FluidityDiagnosticSection`). Périmètre **démos + appels test UNIQUEMENT** (jamais les bénéficiaires — RGPD données de santé).
