...
- `beneficiaries` — profil bénéficiaire + config IA (dont `preferred_engine` : `'openai' | 'gemini'`, défaut OpenAI, choisi dans `/contexte` onglet Configuration IA ; `custom_prompt` TEXT nullable : prompt de personnalité propre au bénéficiaire, copie CONCRÈTE — variables résolues — du défaut, snapshottée à la création par le wizard, éditable dans l'onglet Configuration IA. NULL → fallback sur le défaut `prompt_templates`)
...
| `/admin/appels` | Tous les appels, filtres URL (`?period=`, `?status=`, `?severity=high`). Filtre **Période contextuel** : onglet passés borne `scheduled_at` à `now − X` (libellés « N derniers jours »), onglet prévus à `now + X` (libellés « N prochains jours ») — même clé `period`, libellés différents selon l'onglet. Onglet passés : colonnes Planifié (`scheduled_at`) / Effectif (`started_at`, fallback `notified_at`) + Coût IA + Coût Twilio (réel si remonté, sinon estimation `~` par la durée). Actions : « Relancer » sur missed/failed, « Déclencher maintenant » sur prévus, « Supprimer » (avec confirmation) sur les appels prévus | [AdminAppels.tsx](apps/web/src/pages/admin/AdminAppels.tsx) |
